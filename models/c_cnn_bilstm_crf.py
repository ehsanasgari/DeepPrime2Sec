...
def model_c_cnn_bilstm(n_classes, convs=[3,5,7], dense_size=200, lstm_size=400, drop_lstm=0.5, features_to_use=['onehot','sequence_profile'], filter_size=256):
    '''
    :param max_length:
    :param n_classes:
    ...
    print(model.summary())
    return model, 'model_c_cnn_bilstm_crf#'+'#'.join(features_to_use)+'@conv'+'_'.join([str(c) for c in convs])+'@dense_'+str(dense_size)+'@lstm'+str(lstm_size)+'@droplstm'+str(drop_lstm)+'@filtersize_'+str(filter_size)
